...
**Тестовый метод:** `tests.AeroexpressTest.uc09_aeroexpressSchedule`

**Шаги:**

| # | Действие | Локатор | Ожидание |
|---|---|---|---|
| 1 | Открыть `aeroexpress.tutu.ru` | URL | Body содержит контент (`text.length > 100`) |

**Ассерты:**
- На странице упоминается один из аэропортов / вокзалов: Шереметьево, Внуково, Домодедово, Павелецкий, Белорусский, Киевский.
- Найдено `≥ 3` строк формата `HH:MM`, либо упоминание «расписание»/«отправ».
